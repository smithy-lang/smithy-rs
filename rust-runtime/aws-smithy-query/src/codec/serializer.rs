...
impl ShapeSerializer for QueryShapeSerializer {
    ...
    fn write_list(
        ...
        if !flat {
            let member_name = schema.xml_collection_member_name().unwrap_or("member");
    ...
    fn write_map(
        ...
        let key_name = schema.xml_map_key_name().unwrap_or("key");
        let value_name = schema.xml_map_value_name().unwrap_or("value");
...
mod edge_cases {
    ...
    fn blob_base64_encoded() {
        ...
        let mut ser = QueryShapeSerializer::new("Op", "1.0");
        ser.write_blob(&M, &Blob::new(b"hello".to_vec())).unwrap();
        assert_eq!(
            String::from_utf8(ser.finish()).unwrap(),
