...
static VIRTUAL_HOSTABLE_SEGMENT: Lazy<Regex> = Lazy::new(|| {
    Regex::new("^[a-z\\d][a-z\\d\\-.]{1,61}[a-z\\d]$").unwrap()
});

static IPV4: Lazy<Regex> = Lazy::new(|| {
    Regex::new("^(\\d+\\.){3}\\d+$").unwrap()
});

static DOTS_AND_DASHES: Lazy<Regex> = Lazy::new(|| {
    Regex::new("^.*[.-]{2}.*$").unwrap()
});
...
pub(crate) fn is_virtual_hostable_s3_bucket(host_label: &str, allow_subdomains: bool, e: &mut DiagnosticCollector) -> bool {
    if !is_valid_host_label(host_label, allow_subdomains, e) {
        ...
        host_label.split('.').all(|segment| is_virtual_hostable_segment(segment))
    }
}
...
//         false
//     } else if !allow_subdomains {
//         is_virtual_hostable_s3_segment(host_label)
//     } else {
//         host_label.split('.').all(|segment| is_virtual_hostable_s3_segment(segment))
//     }
// }
//
// fn is_virtual_hostable_s3_segment(host_label: &str) -> bool {
//     (3usize..=63).contains(&host_label.len()) &&
//
//     VIRTUAL_HOSTABLE_SEGMENT.is_match(host_label)
//         && !IPV4.is_match(host_label) // don't allow ip address
//         && !DOTS_AND_DASHES.is_match(host_label) // don't allow names like bucket-.name or bucket.-name
