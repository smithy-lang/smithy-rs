...
//! while still being collected and serialized by the metrics layer. [`ExtensionMetrics`]
...
where
    ...
    T::Closed: Send,
{
    type Rejection = ExtensionMetricsError;
    ...
    fn from_parts(parts: &mut http::request::Parts) -> Result<Self, Self::Rejection> {
        ...
        let Some(metrics_slot) = parts.extensions.get::<MetricsExtension<T>>() else {
            return Err(ExtensionMetricsError::UnknownExtension);
        };
        ...
        let metrics_guard = arc_clone
            ...
            .open(OnParentDrop::Discard)
            .ok_or(ExtensionMetricsError::SlotAlreadyOpened)?;
...
#[derive(Error, Debug)]
pub enum ExtensionMetricsError {
...
impl<Protocol> IntoResponse<Protocol> for ExtensionMetricsError {
