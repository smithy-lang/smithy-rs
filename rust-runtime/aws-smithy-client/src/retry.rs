...
/// Marker that goes in an operation's property bag to communicate that retry is allowed for that operation.
///
/// Only operations that are modeled as `@idempotent`, `@readonly`, or `@retryable` should
/// retry on transient failures. The code generator only includes this marker when creating
/// the operation for operations that are marked by these traits.
#[non_exhaustive]
#[derive(Debug, Default)]
pub struct AllowOperationRetryOnTransientFailure;

impl AllowOperationRetryOnTransientFailure {
    /// Creates a new `AllowOperationRetryOnTransientFailure` marker.
    pub fn new() -> Self {
        Self
    }
}
...
impl RetryHandler {
    ...
    fn should_retry(
        &self,
        transient_failures_retryable: bool,
        ...
        match retry_kind {
            ...
            RetryKind::Error(kind) => {
                if transient_failures_retryable || !matches!(kind, ErrorKind::TransientError) {
                    self.should_retry_error(kind)
                } else {
                    None
                }
            }
    ...
    fn retry_for(
        ...
        transient_failures_retryable: bool,
        retry_kind: RetryKind,
    ) -> Option<BoxFuture<Self>> {
        let (next, dur) = self.should_retry(transient_failures_retryable, &retry_kind)?;
    ...
    R: ClassifyResponse<SdkSuccess<T>, SdkError<E>>,
{
    ...
    fn retry(
        ...
        let transient_failures_retryable = req
            .properties()
            .get::<AllowOperationRetryOnTransientFailure>()
            .is_some();
        let policy = req.retry_policy();
        let retry_kind = policy.classify(result);
        self.retry_for(transient_failures_retryable, retry_kind)
    }
...
mod test {
    ...
    fn eventual_success() {
        ...
        let (policy, dur) = policy
            .should_retry(false, &RetryKind::Error(ErrorKind::ServerError))
            .expect("should retry");
        ...
        let no_retry = policy.should_retry(false, &RetryKind::Unnecessary);
    ...
    fn no_more_attempts() {
        ...
        let no_retry = policy.should_retry(false, &RetryKind::Error(ErrorKind::ServerError));
        assert!(no_retry.is_none());
        assert_eq!(policy.retry_quota(), 490);
    }
    ...
    fn quota_replenishes_on_success() {
        ...
        let policy = Standard::new(conf).new_request_policy(None);
        let (policy, dur) = policy
            .should_retry(true, &RetryKind::Error(ErrorKind::TransientError))
            .expect("should retry");
        assert_eq!(dur, Duration::from_secs(1));
        assert_eq!(policy.retry_quota(), 90);

        let (policy, dur) = policy
            .should_retry(true, &RetryKind::Explicit(Duration::from_secs(1)))
            .expect("should retry");
        assert_eq!(dur, Duration::from_secs(1));
        assert_eq!(
            policy.retry_quota(),
        ...
        assert!(
            policy.should_retry(true, &RetryKind::Unnecessary).is_none(),
    ...
    #[test]
    fn disable_retry_transient_failure() {
        let conf = test_config();
        let policy = Standard::new(conf).new_request_policy(None);
        assert!(policy
            .should_retry(false, &RetryKind::Error(ErrorKind::TransientError))
            .is_none());
    }

    #[test]
    fn enable_retry_transient_failure() {
        let conf = test_config();
        let policy = Standard::new(conf).new_request_policy(None);
        policy
            .should_retry(true, &RetryKind::Error(ErrorKind::TransientError))
