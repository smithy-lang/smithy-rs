...
impl Sign for BasicAuthSigner {
    fn sign_http_request(
        ...
        request.headers_mut().insert(
            http0::header::AUTHORIZATION,
            http0::HeaderValue::from_str(&format!(
                "Basic {}",
...
impl Sign for BearerAuthSigner {
    fn sign_http_request(
        ...
        request.headers_mut().insert(
            http0::header::AUTHORIZATION,
            http0::HeaderValue::from_str(&format!("Bearer {}", token.token())).map_err(|_| {
...
mod tests {
    ...
    fn test_api_key_signing_headers() {
        ...
        let mut request: HttpRequest = http0::Request::builder()
