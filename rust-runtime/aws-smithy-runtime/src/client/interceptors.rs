...
impl<'a, I> Interceptors<I>
where
    I: Iterator<Item = &'a SharedInterceptor>,
{
...
mod tests {
    ...
    fn test_disable_interceptors() {
        ...
        let interceptors = Interceptors::new(rc.interceptors_ref());
        assert_eq!(
            interceptors
                .interceptors
                .filter(|i| i.enabled(&cfg))
                .count(),
            2
        );

        Interceptors::new(rc.interceptors_ref())
