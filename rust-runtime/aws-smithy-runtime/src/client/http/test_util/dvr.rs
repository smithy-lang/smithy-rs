...
impl From<&Request> for http0::Request<()> {
    fn from(request: &Request) -> Self {
        let mut builder = http0::Request::builder().uri(request.uri.as_str());
...
fn headers_to_map_02x(headers: &http0::HeaderMap) -> IndexMap<String, Vec<String>> {
    let mut out: IndexMap<_, Vec<_>> = IndexMap::new();
    for (header_name, header_value) in headers.iter() {
        let entry = out.entry(header_name.to_string()).or_default();
        entry.push(
...
impl<'a, B> From<&'a http0::Response<B>> for Response {
    fn from(resp: &'a http0::Response<B>) -> Self {
...
mod tests {
    ...
    async fn turtles_all_the_way_down() -> Result<(), Box<dyn Error>> {
        ...
        let req = http0::Request::post("https://www.example.com")
        ...
        assert_eq!(
            requests[0].uri(),
            &http0::Uri::from_static("https://www.example.com")
        );
        assert_eq!(
            requests[0].body(),
