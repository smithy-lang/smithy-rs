...
/// let http_client = infallible_client_fn(|_req| http0::Response::builder().status(200).body("OK!").unwrap());
/// ```
pub fn infallible_client_fn<B>(
    f: impl Fn(http0::Request<SdkBody>) -> http0::Response<B> + Send + Sync + 'static,
) -> SharedHttpClient
where
    B: Into<SdkBody>,
{
...
struct InfallibleClientFn {
    ...
    response: Arc<
        dyn Fn(http0::Request<SdkBody>) -> Result<http0::Response<SdkBody>, ConnectorError>
            + Send
            + Sync,
    >,
}
