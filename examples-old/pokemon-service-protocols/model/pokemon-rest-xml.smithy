...
/// A users current Pokemon storage.
resource Storage {
    identifiers: {
        user: String
    }
...
operation GetStorage {
    input := @sensitive @documentation("A request to access Pokemon storage.") {
        @required
        @httpLabel
        user: String
        @required
    ...
    output := @documentation("Contents of the Pokemon storage.") {
        @required
...
/// Not authorized to access Pokemon storage.
@error("client")
@httpError(401)
...
/// Fetch a radio song from the database and stream it back as playable audio.
@readonly
