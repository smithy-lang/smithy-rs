...
B::Error: Into<::aws_smithy_types::body::Error> + 'static,
    ...
    Ok({
        ...
        if let Some(value) = {
                                            let receiver = crate::protocol_serde::shape_capture_pokemon_input::de_events_payload(&mut ::aws_smithy_types::body::SdkBody::from_body_1_x(body))?;
        ...
        input = input.set_region(
                                    crate::protocol_serde::shape_capture_pokemon_input::de_region(m1)?
                                );
        input.build()?
    })
}
...
> {
    Ok({
        ...
        let body = ::aws_smithy_http_server::body::boxed(::http_body_util::StreamBody::new(
            {
                                    let error_marshaller = crate::event_stream_serde::CapturePokemonEventsErrorMarshaller::new();
                                    ...
                                    output.events.into_body_stream(marshaller, error_marshaller, signer)
                                }
        ));
        builder.body(body)?
    })
...
#[allow(clippy::unnecessary_wraps)]
pub fn ser_capture_pokemon_http_error(error: &crate::error::CapturePokemonError) -> std::result::Result<::aws_smithy_http_server::response::Response, ::aws_smithy_http_server::protocol::rest_xml::rejection::ResponseRejection> {
    Ok({
        match error {
            ...
            crate::error::CapturePokemonError::ThrottlingError(output) => {
                let payload = crate::protocol_serde::shape_throttling_error::ser_throttling_error_error(output)?;
                #[allow(unused_mut)]
                let mut builder = ::http_1x::Response::builder();
                builder = ::aws_smithy_http::header::set_response_header_if_absent(
                                builder,
