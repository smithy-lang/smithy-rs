...
if let Some(ref val) = self.field_list {
                    ...
                    ser.write_list(&VALIDATIONERROR_MEMBER_FIELD_LIST, &|ser: &mut dyn ::aws_smithy_schema::serde::ShapeSerializer| {
                        for item in val {
                ...
                pub fn deserialize(deserializer: &mut dyn ::aws_smithy_schema::serde::ShapeDeserializer) -> ::std::result::Result<Self, ::aws_smithy_schema::serde::SerdeError> {
                    ...
                    deserializer.read_struct(&VALIDATIONERROR_SCHEMA, &mut |member, deser| {
                        match member.member_index() {
                            Some(0) => { builder.message = Some(deser.read_string(member)?); }
Some(1) => {
                                    if deser.is_null() { deser.read_null()?; } else {
                                        builder.field_list = Some({ let mut container = Vec::new(); deser.read_list(member, &mut |deser| { container.push(crate::types::ValidationExceptionField::deserialize(deser)?); Ok(()) })?; container });
                                    }
                                }
                            _ => {}
                        }
        ...
        ::std::result::Result::Ok(
            crate::types::error::ValidationError {
                message: self.message
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("message", "message was not specified but it is required when building ValidationError")
                    )?
                ,
