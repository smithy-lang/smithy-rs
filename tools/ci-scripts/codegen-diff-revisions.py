...
def generate_and_commit_generated_code(revision_sha):
    ...
    # Generate code
    run("./gradlew --rerun-tasks aws:sdk:assemble codegen-client-test:assemble codegen-server-test:assemble codegen-server-test:typescript:assemble")
    ...
    run(f"mv codegen-server-test/typescript/build/smithyprojections/codegen-server-test-typescript {OUTPUT_PATH}/")
    ...
    run(f"rm -rf {OUTPUT_PATH}/codegen-server-test-typescript/source")
