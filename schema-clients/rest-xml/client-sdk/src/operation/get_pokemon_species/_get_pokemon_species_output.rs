...
impl  GetPokemonSpeciesOutput  {
    ...
    pub fn flavor_text_entries(&self) -> &[crate::types::FlavorText] {
        use std::ops::Deref; self.flavor_text_entries.deref()
    }
}
                            ...
                            }
{
                    ...
                    ser.write_list(&GETPOKEMONSPECIESOUTPUT_MEMBER_FLAVOR_TEXT_ENTRIES, &|ser: &mut dyn ::aws_smithy_schema::serde::ShapeSerializer| {
                        for item in val {
                ...
                pub fn deserialize(deserializer: &mut dyn ::aws_smithy_schema::serde::ShapeDeserializer) -> ::std::result::Result<Self, ::aws_smithy_schema::serde::SerdeError> {
                    ...
                    deserializer.read_struct(&GETPOKEMONSPECIESOUTPUT_SCHEMA, &mut |member, deser| {
                        match member.member_index() {
                            Some(0) => { builder.name = Some(deser.read_string(member)?); }
Some(1) => { builder.flavor_text_entries = Some({ let mut container = Vec::new(); deser.read_list(member, &mut |deser| { container.push(crate::types::FlavorText::deserialize(deser)?); Ok(()) })?; container }); }
                            _ => {}
                        }
                        Ok(())
                    })?;
...
impl GetPokemonSpeciesOutputBuilder {
    ...
    pub fn set_flavor_text_entries(mut self, input: ::std::option::Option<::std::vec::Vec::<crate::types::FlavorText>>) -> Self {
        self.flavor_text_entries = input; self
    }
    /// A list of flavor text entries for this Pokémon species.
    ...
    pub fn build(self) -> ::std::result::Result<crate::operation::get_pokemon_species::GetPokemonSpeciesOutput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(
            crate::operation::get_pokemon_species::GetPokemonSpeciesOutput {
                name: self.name
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("name", "name was not specified but it is required when building GetPokemonSpeciesOutput")
                    )?
                ,
                flavor_text_entries: self.flavor_text_entries
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("flavor_text_entries", "flavor_text_entries was not specified but it is required when building GetPokemonSpeciesOutput")
                    )?
                ,
            }
        )
