...
class AuthSchemeResolverGenerator(private val codegenContext: ClientCodegenContext) {
        ...
        return RuntimeType.forInlineFun("DefaultAuthSchemeResolver", ClientRustModule.Config.auth) {
            rustTemplate(
                ...
                pub struct DefaultAuthSchemeResolver {
                    ...
                    operation_overrides: std::collections::HashMap<&'static str, Vec<#{AuthSchemeOption}>>,
                }
                ...
                impl #{ServiceSpecificResolveAuthScheme} for DefaultAuthSchemeResolver {
                    fn resolve_auth_scheme<'a>(
                        ...
                        let fut = #{AuthSchemeOptionsFuture}::ready(Ok(modeled_auth_options.clone()));

                        // TODO(AuthAlignment): Incorporate endpoint-based auth options for selected services
    ...
    private fun serviceSpecificResolveAuthSchemeTrait(): RuntimeType {
