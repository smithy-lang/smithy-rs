 ...
 *
 * - two config methods — `emit_input_attributes` (capture + emit on the built-in metrics) and
 *   `capture_input_attributes` (capture only, for in-process reads) — that let the customer name
 *   input members, stored in the config bag as `RequestedTelemetryAttributes`; and
 ...
 *
 * The built-in metrics implementation then carries only the *emitted* members. Off by default: if
 * the customer names nothing, no interceptor does any work and no attribute is captured or emitted.
 *
 * Emitted members become metric labels, so a high-cardinality member (like an object key) fragments
 * the metrics into many low-value time series and inflates cost; the `emit_input_attributes` docs
