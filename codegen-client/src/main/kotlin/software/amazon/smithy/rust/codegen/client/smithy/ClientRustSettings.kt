...
data class ClientCodegenConfig(
    ...
    /**
     * If true, keeps the legacy (`hyper` 0.14 + `rustls` 0.21) client reachable from generated crates:
     *
     * - adds `rustls` as a *default* feature, which selects `aws-smithy-runtime/tls-rustls`
     * - adds an opt-in `legacy-test-util` feature for the pre-1.x test helpers
     * - documents a pre-`v2026_01_12` `BehaviorVersion` in the `behavior_version` example
     *
     * Note this is the setting for clients generated by smithy-rs generally. The AWS SDK does not use it: it
     * always offers the legacy stack through the non-default `legacy-https-client` feature, so opting in is a
     * decision for the consumer of the generated crate rather than a codegen setting.
     */
