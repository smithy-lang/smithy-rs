...
Increasing the MSRV is treated as a minor change rather than a major (breaking) one, per [Cargo's SemVer guidance](https://doc.rust-lang.org/cargo/reference/semver.html#env-new-rust). The MSRV is not increased automatically; when we do increase it, each affected published crate is versioned accordingly: `1.x` crates are increased with a minor release (e.g. `1.4.2` → `1.5.0`), and `0.x` crates are increased with a patch release (e.g. `0.62.1` → `0.62.2`).
