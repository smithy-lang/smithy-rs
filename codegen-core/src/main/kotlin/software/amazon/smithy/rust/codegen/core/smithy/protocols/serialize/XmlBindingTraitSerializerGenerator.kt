...
class XmlBindingTraitSerializerGenerator(
    ...
    private fun RustWriter.serializeMember(memberShape: MemberShape, ctx: Ctx.Scope, rootNameOverride: String? = null) {
        ...
        if (target.isStructureShape && target.members().isEmpty()) {
            // In this case, we use #[allow(unused_variables)] annotation for
            // the "if let Some(...)" block to prevent the warning coming
            // from the following code generated by handleOptional:
            //   if let Some(var_2) = &input.input {
            //       let inner_writer = scope.start_el("input").finish();
            //   }
            // where both var_2 and inner_writer are unused.
            ...
            when (target) {
                ...
                is StructureShape -> {
                    ...
                    if (target.members().isEmpty()) {
                        rust("let inner_writer = ${ctx.scopeWriter}.start_el(${xmlName.dq()})$ns.finish();")
                    } else {
