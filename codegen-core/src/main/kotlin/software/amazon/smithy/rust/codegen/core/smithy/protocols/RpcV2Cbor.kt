...
class RpcV2CborHttpBindingResolver(
    ...
    override fun handlesEventStreamInitialResponse(shape: Shape): Boolean {
        // True if the operation output contains an event stream member as well as non-event stream member
        return when (shape) {
            is OperationShape -> {
                shape.isOutputEventStream(model) && responseBindings(shape).any { it.location == HttpLocation.DOCUMENT }
            }
