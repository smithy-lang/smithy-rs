    ...
    fun `generates valid deserializers`() {
        ...
        project.lib {
            unitTest(
                ...
                let output = ${format(operationGenerator!!)}(json, test_output::op_output::Builder::default()).unwrap().build();
            ...
            unitTest(
                ...
                """
                // empty body
                let output = ${format(operationGenerator)}(b"", test_output::op_output::Builder::default()).unwrap().build();
            ...
            unitTest(
                ...
                let output = ${format(operationGenerator)}(input, test_output::op_output::Builder::default()).unwrap().build();
            ...
            unitTest(
                ...
                """
                // empty error
                let error_output = ${format(errorParser!!)}(b"", test_error::error::Builder::default()).unwrap().build();
            ...
            unitTest(
                ...
                """
                // error with message
                let error_output = ${format(errorParser)}(br#"{"message": "hello"}"#, test_error::error::Builder::default()).unwrap().build();
        ...
        model.lookup<StructureShape>("test#Error").also { error ->
            project.moduleFor(error) {
                error.renderWithModelBuilder(model, symbolProvider, this)
            }
        }
