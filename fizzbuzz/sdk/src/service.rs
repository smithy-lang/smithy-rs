...
/// The service builder for [`FizzBuzzService`].
///
/// Constructed via [`FizzBuzzService::builder`].
pub struct FizzBuzzServiceBuilder<Body, L, HttpPl, ModelPl> {
...
impl<Body, L, HttpPl, ModelPl> FizzBuzzServiceBuilder<Body, L, HttpPl, ModelPl> {
    ...
    ///
    /// ```no_run
    /// use sdk::{FizzBuzzService, FizzBuzzServiceConfig};
    ///
    ...
    /// }
    ///
    /// let config = FizzBuzzServiceConfig::builder().build();
    /// let app = FizzBuzzService::builder(config)
    ///     .fizz_buzz(handler)
    ///     /* Set other handlers */
    ///     .build()
    ///     .unwrap();
    /// # let app: FizzBuzzService<::aws_smithy_http_server::routing::RoutingService<::aws_smithy_http_server::protocol::rpc_v2_cbor::router::RpcV2CborRouter<::aws_smithy_http_server::routing::Route>, ::aws_smithy_http_server::protocol::rpc_v2_cbor::RpcV2Cbor>> = app;
    /// ```
    ///
...
impl<Body, L, HttpPl, ModelPl> FizzBuzzServiceBuilder<Body, L, HttpPl, ModelPl> {
    /// Constructs a [`FizzBuzzService`] from the arguments provided to the builder.
    ///
    /// Forgetting to register a handler for one or more operations will result in an error.
    ///
    /// Check out [`FizzBuzzServiceBuilder::build_unchecked`] if you'd prefer the service to return status code 500 when an
        ...
        L: ::tower::Layer<::aws_smithy_http_server::routing::Route<Body>>,
    {
        ...
        Ok(FizzBuzzService { svc })
    }

    /// Constructs a [`FizzBuzzService`] from the arguments provided to the builder.
    /// Operations without a handler default to returning 500 Internal Server Error to the caller.
    ///
    /// Check out [`FizzBuzzServiceBuilder::build`] if you'd prefer the builder to fail if one or more operations do
    /// not have a registered handler.
    pub fn build_unchecked(self) -> FizzBuzzService<L::Service>
    where
        Body: Send + 'static,
        L: ::tower::Layer<
            ::aws_smithy_http_server::routing::RoutingService<
...
/// The error encountered when calling the [`FizzBuzzServiceBuilder::build`] method if one or more operation handlers are not
/// specified.
#[derive(Debug)]
...
impl std::fmt::Display for MissingOperationsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "You must specify a handler for all operations attached to `FizzBuzzService`.\n\
        ...
        writeln!(f, "\nUse the dedicated methods on `FizzBuzzServiceBuilder` to register the missing handlers:")?;
...
impl FizzBuzzService<()> {
    /// Constructs a builder for [`FizzBuzzService`].
    ...
    >(
        config: FizzBuzzServiceConfig<L, HttpPl, ModelPl>,
    ) -> FizzBuzzServiceBuilder<Body, L, HttpPl, ModelPl> {
    ...
    ///
    /// Use [`FizzBuzzService::builder_without_plugins`] if you don't need to apply plugins.
    ///
    ...
    #[deprecated(
        since = "0.57.0",
        note = "please use the `builder` constructor and register plugins on the `FizzBuzzServiceConfig` object instead; see https://github.com/smithy-lang/smithy-rs/discussions/3096"
    )]
    ...
    >(
        http_plugin: HttpPl,
        model_plugin: ModelPl,
    ) -> FizzBuzzServiceBuilder<Body, ::tower::layer::util::Identity, HttpPl, ModelPl> {
        FizzBuzzServiceBuilder {
            fizz_buzz: None,
    ...
    /// Constructs a builder for [`FizzBuzzService`].
    ///
    /// Use [`FizzBuzzService::builder_with_plugins`] if you need to specify plugins.
    #[deprecated(
        since = "0.57.0",
        note = "please use the `builder` constructor instead; see https://github.com/smithy-lang/smithy-rs/discussions/3096"
    )]
    pub fn builder_without_plugins<Body>() -> FizzBuzzServiceBuilder<
        Body,
...
impl<S> FizzBuzzService<S> {
    /// Converts [`FizzBuzzService`] into a [`MakeService`](tower::make::MakeService).
    ...
    /// Converts [`FizzBuzzService`] into a [`MakeService`](tower::make::MakeService) with [`ConnectInfo`](::aws_smithy_http_server::request::connect_info::ConnectInfo).
    ...
    >
{
    ...
    #[deprecated(
        since = "0.57.0",
        note = "please add layers to the `FizzBuzzServiceConfig` object instead; see https://github.com/smithy-lang/smithy-rs/discussions/3096"
    )]
    pub fn layer<L>(
        self,
        layer: &L,
...
impl<S, R> ::tower::Service<R> for FizzBuzzService<S>
where
    S: ::tower::Service<R>,
{
...
/// An enumeration of all [operations](https://smithy.io/2.0/spec/service-types.html#operation) in FizzBuzzService.
...
impl<L> ::aws_smithy_http_server::service::ContainsOperation<crate::operation_shape::FizzBuzz>
    for FizzBuzzService<L>
{
    const VALUE: Operation = Operation::FizzBuzz;
}

impl<S> ::aws_smithy_http_server::service::ServiceShape for FizzBuzzService<S> {
    const ID: ::aws_smithy_http_server::shape_id::ShapeId =
        ::aws_smithy_http_server::shape_id::ShapeId::new(
            "smithy.example.eventstreaming#FizzBuzzService",
            "smithy.example.eventstreaming",
            "FizzBuzzService",
        );
...
/// Configuration for the [`FizzBuzzService`]. This is the central place where to register and
/// configure [`::tower::Layer`]s, HTTP plugins, and model plugins.
///
/// ```rust,no_run
/// # use sdk::FizzBuzzServiceConfig;
...
pub struct FizzBuzzServiceConfig<L, H, M> {
    layers: L,
    http_plugins: H,
    model_plugins: M,
}

impl FizzBuzzServiceConfig<(), (), ()> {
    /// Returns a builder to construct the configuration.
    pub fn builder() -> FizzBuzzServiceConfigBuilder<
        ::tower::layer::util::Identity,
        ::aws_smithy_http_server::plugin::IdentityPlugin,
        ::aws_smithy_http_server::plugin::IdentityPlugin,
    > {
        FizzBuzzServiceConfigBuilder {
...
/// Builder returned by [`FizzBuzzServiceConfig::builder()`].
#[derive(::std::fmt::Debug)]
pub struct FizzBuzzServiceConfigBuilder<L, H, M> {
    pub(crate) layers: L,
    pub(crate) http_plugins: H,
    pub(crate) model_plugins: M,
}

impl<L, H, M> FizzBuzzServiceConfigBuilder<L, H, M> {
    /// Add a [`::tower::Layer`] to the service.
    pub fn layer<NewLayer>(
        self,
        layer: NewLayer,
    ) -> FizzBuzzServiceConfigBuilder<::tower::layer::util::Stack<NewLayer, L>, H, M> {
        FizzBuzzServiceConfigBuilder {
    ...
    pub fn build(self) -> super::FizzBuzzServiceConfig<L, H, M> {
        super::FizzBuzzServiceConfig {
            layers: self.layers,
            http_plugins: self.http_plugins,
            model_plugins: self.model_plugins,
        }
