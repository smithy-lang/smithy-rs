...
class ServerProtocolTestGenerator(
    ...
    companion object {
        ...
        // TODO This test does not pass, even after fixing it with this function, because we are not honoring
        // setting `X-Amzn-Errortype` header in restJson1: https://awslabs.github.io/smithy/1.0/spec/aws/aws-restjson1-protocol.html#json-shape-serialization
        // The client parses errors from the value of this header, their "code", which is placed in a `aws_smithy_types::error::Error`
        // and then matched against in the function that is generated by `parseError` from `HttpBoundProtocolGenerator`.
        private fun fixRestJsonEmptyComplexErrorWithNoMessage(testCase: HttpResponseTestCase): HttpResponseTestCase =
            testCase.toBuilder().headers(
                mapOf("X-Amzn-Errortype" to "ComplexError")
            ).bodyMediaType(null).build()
