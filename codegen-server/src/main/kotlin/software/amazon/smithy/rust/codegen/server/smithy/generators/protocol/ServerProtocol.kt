...
/** Returns the legacy serde module, or the private protocol serde module owned by one protocol. */
fun serverProtocolSerdeModule(
    protocolId: ShapeId,
    codegenContext: ServerCodegenContext,
): RustModule.LeafModule =
    if (codegenContext.isMultiProtocol) {
        RustModule.private("protocol_serde", parent = serverProtocolRootModule(protocolId))
    } else {
        ProtocolFunctions.defaultSerDeModule(codegenContext)
    }
