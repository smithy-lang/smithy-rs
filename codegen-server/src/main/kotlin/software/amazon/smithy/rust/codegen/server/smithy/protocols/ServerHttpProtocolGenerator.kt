...
private class ServerHttpProtocolImplGenerator(
    ...
    private val codegenScope = arrayOf(
        "ParseStrictRequest" to RuntimeType.parseStrictRequest(runtimeConfig),
        "ParseStrictResponse" to RuntimeType.parseStrictResponse(runtimeConfig),
        "SerializeStrictResponse" to RuntimeType.serializeStrictResponse(runtimeConfig),
        "SerializeStrictError" to RuntimeType.serializeStrictError(runtimeConfig),
    ...
    private fun RustWriter.renderNonStreamingTraits(
        ...
        rustTemplate(
            """
            impl #{ParseStrictRequest} for $operationName {
        ...
        rustTemplate(
            """
            impl #{SerializeStrictResponse} for $operationName {
        ...
        rustTemplate(
            """
            impl #{SerializeStrictError} for $operationName {
                type Output = std::result::Result<#{Http}::Response<#{Bytes}>, #{Error}>;
                type Struct = #{E}Kind;
    ...
    private fun RustWriter.renderStreamingTraits(
        ...
        logger.warning(
            "[rust-server-codegen] $operationShape: streaming trait is not yet implemented"
        )
    }
    ...
    private fun RustWriter.serverRenderShapeSerializer(
        operationShape: OperationShape,
    ...
    private fun serverRenderBindingSerializer(
        ...
        return when (binding.location) {
            HttpLocation.HEADER, HttpLocation.PREFIX_HEADERS, HttpLocation.PAYLOAD -> {
                logger.warning(
                    "[rust-server-codegen] $operationShape: response serialization does not currently support ${binding.location} bindings"
                )
                null
            }
            ...
            HttpLocation.RESPONSE_CODE -> writable {
                rustTemplate(
                    """
                        let status = output.${member.memberName.toLowerCase()}
                            .ok_or(#{Error}::generic(${(member.memberName + " missing or empty").dq()}))?;
    ...
    private fun RustWriter.serverRenderShapeParser(
        ...
        serverRenderPathParser(this, operationShape)
    ...
    private fun generateParseLabelStringFn(binding: HttpBindingDescriptor): RuntimeType {
        ...
        return RuntimeType.forInlineFun(fnName, operationDeserModule) { writer ->
            writer.rustBlockTemplate(
                "##[allow(dead_code)] pub fn $fnName(value: &str) -> std::result::Result<#{O}, #{Error}>",
                *codegenScope,
                "O" to output,
            ) {
                rustTemplate(
