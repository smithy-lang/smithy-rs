...
class ServerSchemaGenerator(
    ...
    private fun isBlobNewtype(target: Shape): Boolean =
        target is BlobShape && isBlobNewtypeType(symbolProvider.toSymbol(target).rustType())

    /**
     * True when a blob-targeting member resolves to a constrained-newtype wrapper. Constraint traits applied to the
     * member itself produce a newtype named after the member (e.g. `input::foo_input::Blob`), which the target
     * shape's symbol does not reveal.
     */
    private fun isBlobNewtypeMember(member: MemberShape): Boolean =
        isBlobNewtypeType(symbolProvider.toSymbol(member).rustType())

    /** A blob newtype is any opaque type other than `aws_smithy_types::Blob` itself; the newtype may also be named `Blob`. */
    private fun isBlobNewtypeType(type: RustType): Boolean {
        var inner = type.stripOuter<RustType.Option>()
        if (inner is RustType.Box) {
            inner = inner.member
        }
        return inner is RustType.Opaque && !(inner.name == "Blob" && inner.namespace?.contains("aws_smithy_types") == true)
    }
    ...
    private fun blobValueExpr(
        ...
        if (isBlobNewtype(target) || (member != null && isBlobNewtypeMember(member))) {
            "$varName.0.clone()"
        } else {
            "$varName.clone()"
        }
    ...
    private fun unionVariantWriteExpr(
        ...
        return when (target) {
            ...
            is BlobShape -> "ser.write_blob(&$memberSchemaRef, ${blobValueExpr(target, varName, member)})?;"
        ...
        return when (target) {
            ...
            is BlobShape ->
                if (target.hasTrait(StreamingTrait::class.java)) {
                    ...
                    "ser.write_blob(&$memberSchemaRef, ${blobValueExpr(target, "val", member)})?;"
                }
