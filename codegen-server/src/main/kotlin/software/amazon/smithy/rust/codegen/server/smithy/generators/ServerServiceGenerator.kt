...
internal class ServerServiceGenerator(
    ...
    private val requestSpecMap: Map<OperationShape, List<RequestSpecInfo>> =
        operations.associateWith { operationShape ->
            requestSpecsForProtocol(operationShape, protocol)
        }
    ...
    private val multiProtocolRequestSpecMap: Map<String, Map<OperationShape, List<RequestSpecInfo>>> by lazy {
        if (!isMultiProtocol) {
            emptyMap()
        } else {
            selectedProtocols.mapIndexed { index, selectedProtocol ->
                ...
                val specsMap =
                    operations.associateWith { operationShape ->
                        requestSpecsForProtocol(operationShape, proto, "_$identifier")
                    }
    ...
    /**
     * Returns the primary request spec and any protocol-specific aliases for an operation.
     *
     * [ServerRpcV2CborProtocol] may register a legacy capitalized alias through
     * [ServerRpcV2CborProtocol.additionalRouterRequestSpecAliases].
     */
    private fun requestSpecsForProtocol(
        operationShape: OperationShape,
        targetProtocol: ServerProtocol,
        functionSuffix: String = "",
    ): List<RequestSpecInfo> {
        val operationName = symbolProvider.toSymbol(operationShape).name
        val requestSpecModule = smithyHttpServer.resolve("routing::request_spec")
        val primarySpec =
            targetProtocol.serverRouterRequestSpec(operationShape, operationName, serviceId.name, requestSpecModule)
        val aliasSpecs =
            if (targetProtocol is ServerRpcV2CborProtocol) {
                targetProtocol.additionalRouterRequestSpecAliases(operationShape, serviceId.name)
            } else {
                emptyList()
            }
        val baseFunctionName = RustReservedWords.escapeIfNeeded(operationName.toSnakeCase()) + functionSuffix
        return (listOf(primarySpec) + aliasSpecs).mapIndexed { index, spec ->
            ...
            val functionBody =
                writable {
                    rustTemplate(
                        ...
                        """,
                        "Spec" to spec,
                        "SpecType" to targetProtocol.serverRouterRequestSpecType(requestSpecModule),
                    )
                }
    ...
    /** Emits route entries for all request specs, cloning the handler only for non-final aliases. */
    private fun RustWriter.emitRouteEntries(
        specFunctions: List<RequestSpecInfo>,
        emitHandlerExpr: RustWriter.(isClone: Boolean) -> Unit,
    ) {
        specFunctions.forEachIndexed { index, specInfo ->
            val isLast = index == specFunctions.size - 1
            rust("($requestSpecsModuleName::${specInfo.functionName}(), ")
                ...
                for (protoInfo in protocolInfos) {
                    ...
                    val routesArrayElements =
                        writable {
                            for (operationShape in operations) {
                                val fieldName = builderFieldNames[operationShape]!!
                                val specFunctions = specsMap.getValue(operationShape)
                                emitRouteEntries(specFunctions) { isClone ->
                                    val routeName = "${fieldName}_${protoInfo.identifier}"
                                    rust(if (isClone) "$routeName.clone()" else routeName)
                                }
                            }
                        }
                    rustTemplate(
    ...
    private fun RustWriter.renderMultiProtocolBuildUncheckedMethod() {
        ...
        rustTemplate(
            ...
            where
                ...
                Body::Error: Into<#{SmithyHttpServer}::error::BoxError>,
                L: #{Tower}::Layer<#{SmithyHttpServer}::routing::Route<Body>>,
                L::Service: Clone,
            {
                #{RouteInitializations:W}
    ...
    private fun requestSpecsModule(): Writable =
        writable {
            val specFunctions =
                if (isMultiProtocol) {
                    multiProtocolRequestSpecMap.values.flatMap { specsMap -> specsMap.values.flatten() }
                } else {
                    requestSpecMap.values.flatten()
                }
            val functions =
                writable {
                    for (specInfo in specFunctions) {
                        rustTemplate(
                            """
                            pub(super) #{Function:W}
                            """,
                            "Function" to specInfo.functionBody,
                        )
                    }
                }
            rustTemplate(
