...
internal object ServerProtocolOrder {
    ...
    fun resolve(
        ...
        val protocolsById = protocols.associateBy { it.protocolShapeId }
        if (protocolsById.size != protocols.size) {
            val duplicateIds =
                protocols
                    .groupingBy { it.protocolShapeId }
                    .eachCount()
                    .filterValues { it > 1 }
                    .keys
        ...
        val selectedProtocolIds = protocolsById.keys
        ...
        if (orderedIds.size != protocols.size) {
            val cycleProtocols =
                incomingCount
                    .filterValues { it > 0 }
                    .keys
        ...
        return orderedIds.map(protocolsById::getValue)
    }
}
