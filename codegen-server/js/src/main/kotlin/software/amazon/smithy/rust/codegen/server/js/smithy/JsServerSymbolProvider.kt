...
class JsServerSymbolVisitor(
    ...
    override fun toSymbol(shape: Shape): Symbol {
        ...
        return if (target is BlobShape && shape.isStreaming(model)) {
            JsServerRuntimeType.byteStream(config().runtimeConfig).toSymbol()
        } else {
            initial
        }
    }
    ...
    override fun timestampShape(shape: TimestampShape?): Symbol {
        return JsServerRuntimeType.dateTime(runtimeConfig).toSymbol()
    }

    override fun blobShape(shape: BlobShape?): Symbol {
        return JsServerRuntimeType.blob(runtimeConfig).toSymbol()
    }

    override fun documentShape(shape: DocumentShape?): Symbol {
        return JsServerRuntimeType.document(runtimeConfig).toSymbol()
    }
}
 ...
 */
class JsStreamingShapeMetadataProvider(private val base: RustSymbolProvider, private val model: Model) : SymbolMetadataProvider(base) {
