...
COMMON_ARGS="--bucket ${BENCHMARK_BUCKET} --region us-east-1"
...
for fs in "tmpfs" "disk"; do
    ...
    cargo run -- --bench put-object --fs "${fs}" --size-bytes 1 ${COMMON_ARGS} &> "${BENCH_RESULT_FILE}"
    aws s3 cp "${BENCH_RESULT_FILE}" "s3://${BENCHMARK_BUCKET}/"

    BENCH_RESULT_FILE="bench_results_get_object_${fs}_1B.txt"
    cargo run -- --bench get-object --fs "${fs}" --size-bytes 1 ${COMMON_ARGS} &> "${BENCH_RESULT_FILE}"
    aws s3 cp "${BENCH_RESULT_FILE}" "s3://${BENCHMARK_BUCKET}/"
done
...
for fs in "tmpfs" "disk"; do
    for size_bytes in "8388607" "8388609" "134217728" "4294967296" "32212254720"; do
        ...
        cargo run -- --bench put-object-multipart --fs "${fs}" --size-bytes "${size_bytes}" --part-size-bytes "8388608" ${COMMON_ARGS} &> "${BENCH_RESULT_FILE}"
        aws s3 cp "${BENCH_RESULT_FILE}" "s3://${BENCHMARK_BUCKET}/"

        BENCH_RESULT_FILE="bench_results_get_object_multipart_${fs}_${size_bytes}B.txt"
        cargo run -- --bench get-object-multipart --fs "${fs}" --size-bytes "${size_bytes}" --part-size-bytes "8388608" ${COMMON_ARGS} &> "${BENCH_RESULT_FILE}"
        aws s3 cp "${BENCH_RESULT_FILE}" "s3://${BENCHMARK_BUCKET}/"
    done
done
