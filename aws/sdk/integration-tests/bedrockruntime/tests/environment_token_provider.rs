...
async fn test_token_configured_with_auth_scheme_preference_also_set_in_env() {
    ...
    let client = aws_sdk_bedrockruntime::Client::new(&shared_config.build());
    let _ = client
        .get_async_invoke()
        .invocation_arn("arn:aws:bedrock:us-west-2:123456789012:invoke/ExampleModel")
        .send()
...
#[tokio::test]
async fn test_explicit_shared_config_takes_precedence() {
    let expected_token = "explicit-code-token";
    let (http_client, captured_request) = capture_request(None);
    let mut shared_config = SdkConfig::builder()
        .region(Region::new("us-west-2"))
        ...
        .token_provider(Token::new(expected_token, None).into_shared());
    shared_config.insert_origin("token_provider", Origin::shared_config());
    let client = aws_sdk_bedrockruntime::Client::new(&shared_config.build());
    let _ = client
        .get_async_invoke()
        .invocation_arn("arn:aws:bedrock:us-west-2:123456789012:invoke/ExampleModel")
