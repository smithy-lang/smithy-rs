...
pub struct ImdsCredentialsProvider {
    ...
    // When `true` (BehaviorVersion < v2026_08_01, or unknown), this provider owns static
    // stability: it extends expiration on stale IMDS credentials and serves the last-retrieved
    // credentials if a refresh fails. When `false` (BV >= v2026_08_01), the `StaticStabilityCache`
    // owns that behavior, so this provider is a plain fetch-once leaf that reports the true expiry
    // and stamps `StaticStabilityEligible`.
    provider_static_stability: bool,
}
...
impl Builder {
    ...
    pub fn build(self) -> ImdsCredentialsProvider {
        ...
        // BV >= v2026_08_01 => the StaticStabilityCache owns serve-cached-on-failure, so this
        // provider drops its own (provider-level) static stability. Older/unknown BV keeps it.
        let provider_static_stability = !provider_config.static_stability_via_cache();
        let client = self
            .imds_override
...
impl ProvideCredentials for ImdsCredentialsProvider {
    ...
    fn fallback_on_interrupt(&self) -> Option<Credentials> {
        // Only meaningful under provider-level static stability (legacy BV). When the cache owns
        // static stability (BV >= v2026_08_01), there is no provider-level fallback.
        if self.provider_static_stability {
...
impl ImdsCredentialsProvider {
    ...
    async fn retrieve_credentials(&self) -> provider::Result {
        ...
        match parse_json_credentials(credentials.as_ref()) {
            Ok(JsonCredentials::RefreshableCredentials(RefreshableCredentials {
                ...
                let _ = account_id;
                // Under provider-level static stability (legacy BV), extend a stale expiration so
                // the credentials remain usable; otherwise report the true expiration and let the
                // StaticStabilityCache decide whether to serve them past expiry.
                let expiration = if self.provider_static_stability {
                    self.maybe_extend_expiration(expiration)
                } else {
                    expiration
                };
        ...
        .map(|mut creds| {
            ...
            // Unconditional: `StaticStabilityEligible` is a passive marker read only by
            // `StaticStabilityCache`, which is installed only for BV >= v2026_08_01. Under older
            // BVs no cache reads it, so stamping is an inert no-op — the cache-install gate is the
            // single point of bifurcation.
    ...
    async fn credentials(&self) -> provider::Result {
        // When the StaticStabilityCache owns static stability (BV >= v2026_08_01), this provider is
        // a plain fetch: surface refresh errors and let the cache serve previously-cached creds.
        if !self.provider_static_stability {
            return self.retrieve_credentials().await;
        }
        // Legacy provider-level static stability: any failure while retrieving credentials MUST NOT
        // impede use of existing credentials.
