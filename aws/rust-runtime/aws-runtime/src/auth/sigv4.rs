...
impl Signer for SigV4Signer {
    fn sign_http_request(
        ...
        let (signing_region, signing_name) = {
            let region = aws_types::region::Region::new(signing_params.region().to_owned());
            let service = signing_params.name().to_owned();

            (SigningRegion::from(region), SigningName::from(service))
        };
            ...
            if let Some(signer_sender) = config_bag.load::<DeferredSignerSender>() {
                ...
                signer_sender
                    .send(Box::new(SigV4MessageSigner::new(
                        _signature,
                        Identity::new(credentials.clone(), credentials.expiry()),
                        signing_region,
                        signing_name,
                        time_source,
                    )) as _)
...
mod event_stream {
    ...
    impl fmt::Debug for SigV4MessageSigner {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            f.debug_struct("SigV4MessageSigner")
...
mod tests {
    ...
    fn endpoint_config_overrides_region_and_service() {
        ...
        let config = Document::Object({
            ...
            out.insert("name".to_owned(), "sigv4".to_owned().into());
            out.insert("signingName".to_owned(), "qldb-override".to_owned().into());
            out.insert(
                "signingRegion".to_owned(),
                "us-east-override".to_owned().into(),
            );
            out
        });
