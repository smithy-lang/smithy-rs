...
/// `Identity::property::<StaticStabilityEligible>()` — no downcast, so it also covers bearer-token
/// identities. Custom/process providers stamp nothing and therefore get caching-only behavior.
///
/// This is a marker property, **not** an [`AwsCredentialFeature`] variant (which is reserved for
/// business metrics).
#[doc(hidden)]
...
impl From<Credentials> for Identity {
    fn from(val: Credentials) -> Self {
        ...
        // Credentials onto the generic Identity, so the cache reads it without a downcast (§4.5).
