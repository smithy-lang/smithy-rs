    ...
    override fun section(section: LibRsSection): Writable = when (section) {
        is LibRsSection.Body -> writable {
            ...
            rustTemplate(
                ...
                "ApiMetadata" to awsHttp(runtimeConfig).resolve("user_agent::ApiMetadata"),
            )

            // Re-export the app name so that it can be specified in config programmatically without an explicit dependency
            rust("pub use #T;", awsTypes(runtimeConfig).resolve("app_name::AppName"))
        }
        else -> emptySection
    }
}
    ...
    override fun section(section: OperationSection): Writable = when (section) {
        is OperationSection.MutateRequest -> writable {
            rustTemplate(
                ...
                """,
                "AwsUserAgent" to awsHttp(runtimeConfig).resolve("user_agent::AwsUserAgent"),
                "Env" to awsTypes(runtimeConfig).resolve("os_shim_internal::Env"),
            )
        }
        else -> emptySection
    }
...
private class AppNameCustomization(runtimeConfig: RuntimeConfig) : ConfigCustomization() {
    private val codegenScope = arrayOf("AppName" to awsTypes(runtimeConfig).resolve("app_name::AppName"))
