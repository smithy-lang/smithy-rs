...
/// For this example, we would be returning the errors as a variant
/// of CustomError data type.
...
/// An interceptor that will ensure all outgoing requests have a specific
/// header name present, otherwise it will return an error.
...
impl Interceptor for CheckHeaderInterceptor {
    ...
    fn read_before_signing(
            ...
            .any(|(ref header_name, _)| *header_name == HEADER_TO_CHECK)
        {
            ...
            return Err(Box::new(CustomError::HeaderMissing(
    ...
    match response_result {
        ...
        Err(SdkError::DispatchFailure(de)) => {
            ...
            let custom_error = de
                ...
                .and_then(|custom_error| custom_error.downcast_ref::<CustomError>());

            if let Some(custom_error) = custom_error {
                match custom_error {
                    e @ CustomError::HeaderMissing(_missing_header) => {
                        // While we're only logging the error in this instance, but we can implement
