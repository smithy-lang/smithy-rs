// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
/* JsonSerializerGenerator.kt:215 */
pub fn ser_storage_access_not_authorized_error(
    value: &crate::error::StorageAccessNotAuthorized,
) -> ::std::result::Result<String, ::aws_smithy_types::error::operation::SerializationError> {
    /* JsonSerializerGenerator.kt:220 */
    let mut out = ::std::string::String::new();
    let mut object = ::aws_smithy_json::serialize::JsonObjectWriter::new(&mut out);
    /* JsonSerializerGenerator.kt:377 */
    crate::protocol_aws_json1_1::protocol_serde::shape_storage_access_not_authorized::ser_storage_access_not_authorized(&mut object, value)?;
    /* ServerAwsJson.kt:95 */
    object.key("__type").string("StorageAccessNotAuthorized");
    /* JsonSerializerGenerator.kt:229 */
    object.finish();
    Ok(out)
    /* JsonSerializerGenerator.kt:215 */
}

/* JsonSerializerGenerator.kt:360 */
pub fn ser_storage_access_not_authorized(
    #[allow(unused_variables)] object: &mut ::aws_smithy_json::serialize::JsonObjectWriter,
    #[allow(unused_variables)] input: &crate::error::StorageAccessNotAuthorized,
) -> ::std::result::Result<(), ::aws_smithy_types::error::operation::SerializationError> {
    /* JsonSerializerGenerator.kt:374 */
    Ok(())
    /* JsonSerializerGenerator.kt:360 */
}
