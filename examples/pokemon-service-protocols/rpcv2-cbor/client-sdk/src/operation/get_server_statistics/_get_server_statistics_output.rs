// Code generated by software.amazon.smithy.rust.codegen.smithy-rs. DO NOT EDIT.
#[allow(missing_docs)] // documentation missing in model
#[non_exhaustive]
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::fmt::Debug)]
pub struct GetServerStatisticsOutput  {
    /// The number of calls executed by the server.
    pub calls_count: i64,
}
impl  GetServerStatisticsOutput  {
    /// The number of calls executed by the server.
    pub fn calls_count(&self) -> i64 {
        self.calls_count
    }
}
static GETSERVERSTATISTICSOUTPUT_MEMBER_CALLS_COUNT: ::aws_smithy_schema::Schema<'static> = ::aws_smithy_schema::Schema::new_member(
                            ::aws_smithy_schema::ShapeId::from_parts(
                                "com.aws.example.synthetic#GetServerStatisticsOutput$calls_count",
                                "com.aws.example.synthetic",
                                "GetServerStatisticsOutput",
                            ),
                            ::aws_smithy_schema::ShapeType::Long,
                            "calls_count",
                            0,
                        );
static GETSERVERSTATISTICSOUTPUT_SCHEMA: ::aws_smithy_schema::Schema<'static> = ::aws_smithy_schema::Schema::new_struct(
                            ::aws_smithy_schema::ShapeId::from_parts("com.aws.example.synthetic#GetServerStatisticsOutput", "com.aws.example.synthetic", "GetServerStatisticsOutput"),
                            ::aws_smithy_schema::ShapeType::Structure,
                            &[&GETSERVERSTATISTICSOUTPUT_MEMBER_CALLS_COUNT],
                        )
    .with_original_name("GetServerStatisticsOutput");
impl GetServerStatisticsOutput {
                /// The schema for this shape.
                pub const SCHEMA: &'static ::aws_smithy_schema::Schema<'static> = &GETSERVERSTATISTICSOUTPUT_SCHEMA;
            }
impl ::aws_smithy_schema::serde::SerializableStruct for GetServerStatisticsOutput {
                #[allow(unused_variables, clippy::diverging_sub_expression)]
                fn serialize_members(&self, ser: &mut dyn ::aws_smithy_schema::serde::ShapeSerializer) -> ::std::result::Result<(), ::aws_smithy_schema::serde::SerdeError> {
                    {
                                let val = &self.calls_count;
                                ser.write_long(&GETSERVERSTATISTICSOUTPUT_MEMBER_CALLS_COUNT, *val)?;
                            }
                    Ok(())
                }
            }
impl GetServerStatisticsOutput {
                /// Deserializes this structure from a [`ShapeDeserializer`].
                pub fn deserialize(deserializer: &mut dyn ::aws_smithy_schema::serde::ShapeDeserializer) -> ::std::result::Result<Self, ::aws_smithy_schema::serde::SerdeError> {
                    #[allow(unused_variables, unused_mut)]
                    let mut builder = Self::builder();
                    #[allow(unused_variables, unreachable_code, clippy::single_match, clippy::match_single_binding, clippy::diverging_sub_expression)]
                    deserializer.read_struct(&GETSERVERSTATISTICSOUTPUT_SCHEMA, &mut |member, deser| {
                        match member.member_index() {
                            Some(0) => { builder.calls_count = Some(deser.read_long(member)?); }
                            _ => {}
                        }
                        Ok(())
                    })?;
                    builder.calls_count = builder.calls_count.or(Some(0i64));
builder.build().map_err(|e| aws_smithy_schema::serde::SerdeError::custom(e.to_string()))
                }
            }
impl GetServerStatisticsOutput {
                        /// Deserializes this structure from a body deserializer and HTTP response.
                        pub fn deserialize_with_response(
                            deserializer: &mut dyn ::aws_smithy_schema::serde::ShapeDeserializer,
                            _headers: &::aws_smithy_runtime_api::http::Headers,
                            _status: u16,
                            _body: &[u8],
                        ) -> ::std::result::Result<Self, ::aws_smithy_schema::serde::SerdeError> {

                            Self::deserialize(deserializer)
                        }
                    }
impl GetServerStatisticsOutput {
    /// Creates a new builder-style object to manufacture [`GetServerStatisticsOutput`](crate::operation::get_server_statistics::GetServerStatisticsOutput).
    pub fn builder() -> crate::operation::get_server_statistics::builders::GetServerStatisticsOutputBuilder {
        crate::operation::get_server_statistics::builders::GetServerStatisticsOutputBuilder::default()
    }
}

/// A builder for [`GetServerStatisticsOutput`](crate::operation::get_server_statistics::GetServerStatisticsOutput).
#[derive(::std::clone::Clone, ::std::cmp::PartialEq, ::std::default::Default, ::std::fmt::Debug)]
#[non_exhaustive]
pub struct GetServerStatisticsOutputBuilder {
    pub(crate) calls_count: ::std::option::Option<i64>,
}
impl GetServerStatisticsOutputBuilder {
    /// The number of calls executed by the server.
    /// This field is required.
    pub fn calls_count(mut self, input: i64) -> Self {
        self.calls_count = ::std::option::Option::Some(input);
        self
    }
    /// The number of calls executed by the server.
    pub fn set_calls_count(mut self, input: ::std::option::Option<i64>) -> Self {
        self.calls_count = input; self
    }
    /// The number of calls executed by the server.
    pub fn get_calls_count(&self) -> &::std::option::Option<i64> {
        &self.calls_count
    }
    /// Consumes the builder and constructs a [`GetServerStatisticsOutput`](crate::operation::get_server_statistics::GetServerStatisticsOutput).
    /// This method will fail if any of the following fields are not set:
    /// - [`calls_count`](crate::operation::get_server_statistics::builders::GetServerStatisticsOutputBuilder::calls_count)
    pub fn build(self) -> ::std::result::Result<crate::operation::get_server_statistics::GetServerStatisticsOutput, ::aws_smithy_types::error::operation::BuildError> {
        ::std::result::Result::Ok(
            crate::operation::get_server_statistics::GetServerStatisticsOutput {
                calls_count: self.calls_count
                    .ok_or_else(||
                        ::aws_smithy_types::error::operation::BuildError::missing_field("calls_count", "calls_count was not specified but it is required when building GetServerStatisticsOutput")
                    )?
                ,
            }
        )
    }
}

