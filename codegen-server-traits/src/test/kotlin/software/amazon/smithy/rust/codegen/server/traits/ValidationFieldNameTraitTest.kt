...
class ValidationFieldNameTraitTest {
    ...
    fun testValidationFieldNameTrait() {
        ...
        assertEquals(ShapeId.from("smithy.rust.codegen.server.traits#validationFieldName"), trait.toShapeId())

        // Test the Provider
        val provider = ValidationFieldNameTrait.Provider()
        assertEquals(ShapeId.from("smithy.rust.codegen.server.traits#validationFieldName"), provider.shapeId)
    }
}
